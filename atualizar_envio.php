...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sqlUpdate = "UPDATE envio_farinha SET nome=?, fornecedores=?, numero_romaneio=?, data=?, peso_ova=?, saca_ova=?, peso_torrada=?, saca_torrada=?, peso_file=?, saca_file=?, lotes=?, quantidade_sacas=?, peso_total=?, observacoes=? WHERE id=?";
    $stmt = $conn->prepare($sqlUpdate);

    // Corrigindo a string de tipos para corresponder ao número de parâmetros
    $stmt->bind_param("ssssiiiiiissssi", $nome, $fornecedores, $numero_romaneio, $data, $peso_ova, $saca_ova, $peso_torrada, $saca_torrada, $peso_file, $saca_file, $lotes, $quantidade_sacas, $peso_total, $observacoes, $id);
